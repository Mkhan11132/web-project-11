...
app.post('/register', async (req, res) => {
  try {
    ...
    const [existingUsers] = await db.query("SELECT * FROM users WHERE email=?", [email]);
    if (existingUsers.length > 0) return res.send("Email already registered");
    ...
    if (ref) {
      const extractCode = ref.match(/REF\d+/);
      const cleanRef = extractCode ? extractCode[0] : ref;
